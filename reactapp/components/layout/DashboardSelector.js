...
import { useLayoutContext, useLayoutNameContext } from "components/contexts/SelectedDashboardContext";
...
import { BsArrowReturnLeft, BsSave, BsPencilSquare, BsFileText, BsTrash } from "react-icons/bs";
...
function DashboardSelector() {
    ...
    const [showAddDashboardModal, setShowAddDashboardModal] = useAddDashboardModalShowContext();
    const [showNotesModal, setShowNotesModal] = useDashboardNotesModalShowContext();
    const [isEditing, setIsEditing] = useEditingContext();
    const {csrf} = useContext(AppContext);
  
    useEffect(() => {
      appAPI.getDashboards().then((data) => {
        let options = [{value: "Create a New Dashboard", label: "Create a New Dashboard", color: 'rgb(156, 156, 156, .5)'}]
      ...
      <>
        ...
        {selectedOption &&
          <>
            {isEditing &&
                <>
                    <HeaderButton tooltipPlacement="bottom" tooltipText="Cancel Changes" onClick={onCancel}>
                      <BsArrowReturnLeft size="1.5rem"/>
                    </HeaderButton>
                    <HeaderButton tooltipPlacement="bottom" tooltipText="Save Changes" form="rowUpdate" type="submit">
                      <BsSave size="1.5rem"/>
                    </HeaderButton>
                </>
            }
            {!isEditing && 
              <HeaderButton tooltipPlacement="bottom" tooltipText="Edit Dashboard" onClick={onEdit}>
                <BsPencilSquare size="1.5rem"/>
              </HeaderButton>}
            <HeaderButton tooltipPlacement="bottom" tooltipText="Open Notes" onClick={onNotes}>
              <BsFileText size="1.5rem"/>
            </HeaderButton>
            <HeaderButton tooltipPlacement="bottom" tooltipText="Delete Dashboard" onClick={onDelete}>
              <BsTrash size="1.5rem"/>
            </HeaderButton>
          </>
        }
