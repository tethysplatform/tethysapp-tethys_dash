...
//
// Three things are registered, from three different places, and the split
// matters:
//
...
//
//   3. Every other EPSG code, from the generated table -- see
//      ensureProjectionAsync. A raster names its CRS by code and carries no
//      definition either, and there is no curating a table by hand that covers
//      the codes the world's data is published in.
//
// The curated table is what is on hand *synchronously*, which is why it is
// short and why it still exists: case 3 arrives as a chunk that has to be
// fetched, and `ensureProjection` has callers that cannot await. A survey of the
// live dashboards found exactly one layer naming a non-native code (a WMS layer
// requesting EPSG:5041); EPSG:5070 is included because US national hydrology
// datasets commonly name Conus Albers by code. Adding a zone is a table entry
// plus a control point, and `ensureProjection` registers it on demand rather
// than at startup.
//
...
// play. That cost is also why the generated table is never registered wholesale
// -- only the one code a layer asked for is declared. It, plus proj4 and
// wkt-parser themselves at ~150 KiB, is why nothing imports this module
// statically -- the map awaits it only when a layer could carry or name a
// non-native CRS, and the shapefile pipeline loads it from inside its own async
// read. Importing this module registers INITIAL_CODES as a side effect (see the
// bottom of the file), which is what puts a table code on hand for a layer that
// names one and nothing else asks for.
...
// The generated EPSG table, holding every EPSG code whose definition was checked
// against PROJ's own answer when the table was built and agreed with it to
// within the tolerance the artifact records. Generated by `npm run
// generate:epsg`; see scripts/epsg/.
//
// It is a chunk of its own -- ~640 KiB of JSON, ~75 KiB over the wire -- fetched
// the first time a layer names a code that neither OpenLayers nor the curated
// table above covers, and never at all for a dashboard whose layers are in Web
// Mercator or a WGS84 UTM zone. The promise is memoized rather than the table,
// so concurrent layers share one fetch.
...
// Why a code the generator knew about was left out of the table. The generator
// ships only the definitions it could reproduce and records the rest, so a
// layer that names one can be told what is wrong with it instead of being told
// the code is unknown.
const UNSUPPORTED_REASONS = {
  method:
    "uses a projection method this browser's projection engine does not implement",
  axis: "uses a south- or west-orientated axis order this browser's projection engine does not implement",
  inaccurate:
    "cannot be reproduced in the browser closely enough to be drawn in the right place",
};
 ...
 *
 * Returns a result rather than null-or-projection, which is what separates it
 * from `ensureProjection` beyond being async: a code can fail here for reasons
 * an author can act on, and the caller turns them into the layer's error.
 *
 * A generated definition is registered without probing it. `ensureProjection`
 * and `registerProjectionDefinition` probe because a definition reaching them
 * is unseen -- a table entry someone hand-wrote, or WKT out of a file. These
 * were transformed against PROJ at five points across their own area of use
 * before being written, which is a stronger check than the probe and one that
 * has already happened.
 *
 * @param {string} code Projection code, e.g. "EPSG:5629".
 * @returns {Promise<{projection: import("ol/proj/Projection.js").default}|{error: {reason: string, detail: string}}>}
 */
export async function ensureProjectionAsync(code) {
  if (typeof code !== "string" || code.trim() === "") {
    return {
      error: { reason: "empty", detail: "No projection code was given." },
    };
  }
  ...
  if (!epsg) {
    return {
      error: {
        reason: "unknown",
        detail: `"${trimmed}" is not an EPSG code, and nothing has registered it. ${ADVICE}`,
      },
    };
  }
  ...
  if (definition) {
    // Re-check the registry: another layer may have resolved the same code
    // while this one awaited the chunk, and re-declaring would rebuild every
    // pairwise transform for nothing.
    const existing = getProjection(normalized);
    if (existing) return { projection: existing };
    proj4.defs(normalized, definition);
    register(proj4);
    return { projection: getProjection(normalized) };
  }

  const dropped = table.unsupported[id];
  if (dropped) {
    const [reason, name] = dropped;
    return {
      error: {
        reason,
        detail: `${normalized} (${name}) ${UNSUPPORTED_REASONS[reason]}. ${ADVICE}`,
      },
    };
  }
  ...
  return {
    error: {
      reason: "unknown",
      detail: `${normalized} is not in the EPSG ${table.epsgVersion} table this build carries. ${ADVICE}`,
    },
  };
}
