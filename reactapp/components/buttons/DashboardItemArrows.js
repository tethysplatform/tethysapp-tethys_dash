  ...
  const StyledDropdownButton = (
    <StyledDropdown>
      <StyledDropdown.Toggle
        variant="info"
        id="cellUpdateDropdown"
        style={{ width: "100%", height: "100%" }}
      >
        {arrowDirection === "up" ? (
          <StyledUpIcon size="1.5rem" />
        ) : arrowDirection === "right" ? (
          <StyledRightIcon size="1.5rem" />
        ) : arrowDirection === "down" ? (
          <StyledDownIcon size="1.5rem" />
        ) : (
      ...
      <StyledDropdown.Menu>
        {arrowDirection === "up" || arrowDirection === "down" ? (
          <>
            <StyledDropdown.Item onClick={addRow}>
              Add Row {arrowDirection === "up" ? "Above" : "Below"}
            </StyledDropdown.Item>
            {((arrowDirection === "up" && rowNumber !== 0) ||
              (arrowDirection === "down" &&
                rowNumber !== rowData.length - 1)) && (
              <StyledDropdown.Item onClick={moveRow}>
                Move Row {arrowDirection === "up" ? "Above" : "Below"}
              </StyledDropdown.Item>
            )}
          </>
        ) : (
          <>
            <StyledDropdown.Item onClick={addColumn}>
              Add Cell on {arrowDirection === "right" ? "Right" : "Left"}
            </StyledDropdown.Item>
            {((arrowDirection === "left" && colNumber !== 0) ||
              (arrowDirection === "right" &&
                colNumber !== rowData[rowNumber]["columns"].length - 1)) && (
              <StyledDropdown.Item onClick={moveColumn}>
                Move Cell {arrowDirection === "right" ? "Right" : "Left"}
              </StyledDropdown.Item>
            )}
          </>
        )}
