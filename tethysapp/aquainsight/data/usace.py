...
def parse_usace_data(data):
    ...
    dates = [pd.to_datetime(date.replace("T24", "T00")).tz_convert("UTC") + timedelta(days=1)  if re.findall(".*-(.*)T24", date) else pd.to_datetime(date).tz_convert("UTC") for date in dates]
    df['Datetime'] = dates
