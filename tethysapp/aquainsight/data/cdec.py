...
def get_station_data(station, start, end, duration, data_type):
    ...
    data_url = f"https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations={station}"
...
def get_cdec_data(station, start, end):
    ...
    cdec_data = pd.merge(elevation_data, storage_data, left_index=True, right_index=True)
    ...
    return JsonResponse({
        'series': series,
        'title': f"{station_metadata['stationName']} Hourly Data"
    })