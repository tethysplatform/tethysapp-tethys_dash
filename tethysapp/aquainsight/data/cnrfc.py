...
def get_hefs_data(location, rating_curve_flows, rating_curve_stages):
    ...
    try:
        ...
        unit = "cfs"
        hefs_csv_url = f"https://www.cnrfc.noaa.gov/csv/{location}_hefs_csv_hourly.csv"
        df = pd.read_csv(hefs_csv_url)
    df = df.drop(0)
    df = df.set_index("GMT")
    df = df.astype(float)
    ...
    for ens in ens_columns:
        ensemble_series.append({"title": ens, "x": dates, "y": df_stage[ens].tolist()})
    hefs_series = {
...
def get_hefs_metadata(location):
    ...
    hefs_plot_web = f"https://www.cnrfc.noaa.gov/ensembleProduct.php?id={location}"
...
def get_hydro_thresholds(rating_curve_flows, rating_curve_stages, charting_data):
    ...
    for threshold in re.findall(
        ...
        hydro_thresholds.append(
            {
                "name": threshold_json["label"]["text"]
                + f" ({round(interpolated_flow, 2)} cfs)",
