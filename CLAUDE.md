...
- **LLM-editability of plugin args**: Intake and `client_custom` plugin args are LLM-editable by default via the chatbox's `patch_visualization` tool. Matches the edit-modal permission model — editors can set any arg there too. Plugin authors opt out per-arg via the `llm_non_editable_args` class attribute (Intake) or `llmNonEditableArgs` in `package.json` (client_custom) when they need to protect a specific arg (e.g., hardcoded credential). See `docs/source/plugins.rst` "LLM-editability" section and `tethysdash inspect_editable_paths <source>` for author-facing inspection.
