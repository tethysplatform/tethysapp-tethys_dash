...
- **LLM-editability of plugin args**: Intake and `client_custom` plugin args are LLM-editable by default via the chatbox's `patch_visualization` tool. A mandatory project-wide sensitive-name pattern deny-list (credentials / network / filesystem / injection-prone suffixes) is applied last and overrides plugin author declarations. Plugin authors opt out of individual args via the `llm_non_editable_args` class attribute (Intake) or `llmNonEditableArgs` in `package.json` (client_custom). See `docs/source/plugins.rst` "LLM-editability" section and `tethysdash inspect_editable_paths <source>` for author-facing inspection.
